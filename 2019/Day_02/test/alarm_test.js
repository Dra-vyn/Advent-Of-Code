...
Deno.test("input : 1,0,0,0,99" , () =>
  assertEquals(allfns.test("1,0,0,0,99"), "2,0,0,0,99"));

Deno.test("input : 2,3,0,3,99", () =>
  assertEquals(allfns.test("2,3,0,3,99"), "2,3,0,6,99"));

Deno.test("input : 2,4,4,5,99,0", () =>
  assertEquals(allfns.test("2,4,4,5,99,0"), "2,4,4,5,99,9801"));

Deno.test("input : 1,1,1,4,99,5,6,0,99", () =>
  assertEquals(
    allfns.test("1,1,1,4,99,5,6,0,99"),
    "30,1,1,4,2,5,6,0,99",
  ));

Deno.test("main test", () => {
  const input = Deno.readTextFileSync("Data/input.txt");
  assertEquals(allfns.part1(input, [[1, 12], [2, 2]]), 3706713)
});
